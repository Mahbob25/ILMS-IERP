...
async def list_closures(
    ...
    refunds_dates = select(func.date(Refund.disbursed_at).distinct().label("date")).subquery()
    union_query = select(payments_dates.c.date).union(
        select(expenses_dates.c.date),
        select(refunds_dates.c.date),
    ...
    query = select(
        ...
        func.coalesce(
            ...
            .where(func.date(Refund.disbursed_at) == union_query.c.date)
            .correlate(union_query)
            .scalar_subquery(), 0
        ).label("total_refunds_out"),
    ...
    return [
        {
            ...
            "total_refunds_out": float(row.total_refunds_out or 0),
            "net_cash_flow": float((row.total_payments_in or 0) - (row.total_expenses_out or 0) - (row.total_refunds_out or 0)),
        }
        for row in rows
    ]
...
async def get_daily_ledger(db: AsyncSession, ledger_date: date) -> dict:
    ...
    refunds_detail_result = await db.execute(
        select(
            Refund.id,
            Refund.amount,
        ...
        .outerjoin(Employee, User.employee_id == Employee.id)
        .where(func.date(Refund.disbursed_at) == ledger_date)
        .order_by(Refund.receipt_number)
    )
    ...
    refunds_detail = [
        {
            ...
            "disbursed_by_name": row[8] or "",
        }
        for row in refunds_detail_result.fetchall()
    ]
    ...
    return {
        ...
        "total_refunds_out": total_refunds_out,
