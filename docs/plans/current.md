...
- **Portal dashboards rebuilt as "the Register"** (2026-09-12) — the student and parent dashboards are now a single ruled *standing sheet* built from the attendance register the institute already uses: an ink standfirst band with three ledger figures (attendance / course average / outstanding), a signature ribbon of one mark per real session grouped by month, courses as transcript rows each carrying a 5-mark micro ribbon, fees as a ledger line with the last receipts, institute announcements, and an honest not-yet-wired AI entry. Guardians with more than one child get every child's standing at a glance plus a focus switcher; a student sees only their own sheet. No chart library — the marks are flex spans that inherit `dir`, because recharts has no RTL axis handling anywhere in this repo (dashboard route ~6.4 kB, recharts out of the portal bundle). Also restored: the portal data layer a previous revert had removed (`git revert acf8967`) — the student self-view resolver, `/me/fees`, the richer `SectionDTO`, the `partial` attendance status, `/me/announcements`, and the My Courses page. The rejected UI that came with it (`StatCard`, `AttendanceDonut`) was deleted, and `/dashboard/ai/explain` now requires a course instead of silently 400-ing. Residual gaps are recorded in `operations/known-issues.md`.
