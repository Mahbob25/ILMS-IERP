...
- **Caddy Internal CA:** يستخرج Root CA محلي ويوقّع شهادات HTTPS لـ `lims.institute.local`. يُصدِّر Root CA إلى أجهزة العميل عبر صفحة تنزيل مؤمَّنة (أو توزيع يدوي). هذا يُفعّل `Secure` على الكوكيز ويُنهي الحاجة لأي استثناء متصفح.
...
- **السبب التقني:** Caddy يعمل كـ **Internal CA** ويوقّع شهادات HTTPS موثوقة محلياً لـ `lims.institute.local` (مع تصدير Root CA لأجهزة العميل). النتيجة:
...
sequenceDiagram
    ...
    U->>F: إدخال بيانات الدخول (https://lims.institute.local)
...
- **إعدادات الكوكيز النهائية:**
    ...
    - `SameSite=Lax`: ✅
    - `Path=/`: ✅
    - `Domain`: يُترك فارغاً (الكوكيز خاصة بنطاق LAN المحلي، `lims.institute.local`).
...
ingress:
  ...
  - hostname: admin.lims.institute.local
...
- التطبيق: `admin.lims.institute.local`
...
- **Cloudflare Access (Zero Trust):** إعداد Tunnel + Policies (Email OTP + MFA) لـ `admin.lims.institute.local`.
