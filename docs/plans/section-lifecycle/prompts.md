...
## Prompt Template for Parallel Agents (Phases 2, 3, 4, 5, 6)

Use this template. Replace the placeholders for each phase.

```markdown
You are implementing {{PHASE_NAME}} — {{PHASE_TITLE}} for the Section Lifecycle project.
...
## What to Read
Read `docs/plans/section-lifecycle/phase-{{PHASE_FILE}}.md` — it contains all the implementation details for this phase.
...
|-------------|-------|
| `{{PHASE_NAME}}` | Phase 2 |
| `{{PHASE_TITLE}}` | Startup Daily Checks |
| `{{PHASE_FILE}}` | 02-startup-daily-checks |
| `{{ESTIMATE}}` | 1.25 |
| `{{CREATE_FILES}}` | `backend/app/modules/academic/section_startup_checks.py` |
| `{{EDIT_FILES}}` | `backend/app/main.py` (wire lifespan call) |
| `{{BOUNDARIES}}` | - `service.py`, `ledger_service.py`, `cancellation_service.py`, `cashier_service.py` (no business logic mutations) - Do NOT create API endpoints - Do NOT perform financial mutations (read-only flag updates only) |
...
|-------------|-------|
| `{{PHASE_NAME}}` | Phase 3 |
| `{{PHASE_TITLE}}` | Grade & Payment Enforcement |
| `{{PHASE_FILE}}` | 03-grade-payment-enforcement |
| `{{ESTIMATE}}` | 1.75 |
| `{{CREATE_FILES}}` | None |
| `{{EDIT_FILES}}` | `backend/app/modules/academic/service.py` (edit `complete_section()`), `backend/app/modules/academic/router.py` (add force/reason params, restrict DELETE) |
| `{{BOUNDARIES}}` | - `section_startup_checks.py` (Phase 2 concern) - `cancellation_service.py`, `cashier_service.py` (Phase 4 concern) - `ledger_service.py` (Phase 5 concern) - Do NOT add new API endpoints (only modify existing ones) |
...
|-------------|-------|
| `{{PHASE_NAME}}` | Phase 4 |
| `{{PHASE_TITLE}}` | Cancellation Backend |
| `{{PHASE_FILE}}` | 04-cancellation-backend |
| `{{ESTIMATE}}` | 4.25 |
| `{{CREATE_FILES}}` | `backend/app/modules/academic/cancellation_service.py`, `backend/app/modules/lms/cashier_service.py` |
| `{{EDIT_FILES}}` | `backend/app/modules/academic/router.py` (append 3 manager endpoints), `backend/app/modules/lms/router.py` (append 4 cashier endpoints) |
| `{{BOUNDARIES}}` | - `section_startup_checks.py` (Phase 2 concern) - `service.py` → `complete_section()` (Phase 3 concern) - `ledger_service.py` deactivation code (Phase 5 concern) - Do NOT modify grade/payment enforcement logic - Do NOT touch frontend |
...
|-------------|-------|
| `{{PHASE_NAME}}` | Phase 5 |
| `{{PHASE_TITLE}}` | Deactivation |
| `{{PHASE_FILE}}` | 05-deactivation |
| `{{ESTIMATE}}` | 1.25 |
| `{{CREATE_FILES}}` | None |
| `{{EDIT_FILES}}` | `backend/app/modules/lms/ledger_service.py` (add `deactivate_contract()`), `backend/app/modules/academic/service.py` (append `deactivate_section()`), `backend/app/modules/academic/router.py` (append deactivate endpoint) |
| `{{BOUNDARIES}}` | - `section_startup_checks.py` (Phase 2 concern) - `complete_section()` function in service.py (Phase 3 concern — append new functions only) - `cancellation_service.py`, `cashier_service.py` (Phase 4 concern) |
...
| `{{PHASE_FILE}}` | 06-frontend |
| `{{ESTIMATE}}` | 5.75 |
| `{{CREATE_FILES}}` | All new UI components (see phase doc for full list) |
| `{{EDIT_FILES}}` | Existing section list/detail pages, student profile, i18n translation files |
| `{{BOUNDARIES}}` | - Do NOT create or modify any backend API endpoints - Do NOT modify database schema or models - Do NOT touch any Python backend file - Work against the API contract from Phase 1; use mock data if backends aren't ready yet |
