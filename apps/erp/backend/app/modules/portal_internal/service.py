...
from app.modules.content.service import list_announcements
...
async def get_announcements(db: AsyncSession) -> list[dict[str, Any]]:
    """Active institute-wide announcements, in display order.

    Not student-scoped: every portal account sees the same notices, so there is
    no actor → student check here. Reuses the content module's query so the
    portal and the marketing site can never disagree on what is published.
    """
    rows = await list_announcements(db, active_only=True)
    return [
        {
            "id": r.id,
            "text_ar": r.text_ar,
