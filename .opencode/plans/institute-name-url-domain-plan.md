...
### Phase 1: Config & Infrastructure (lims.institute.local → aldirasat.edu)

**1. `apps/erp/backend/app/core/config.py:13`**
```
- CORS_ORIGINS: str = "https://lims.institute.local"
+ CORS_ORIGINS: str = "https://aldirasat.edu"
```
...
### Phase 2: Backend Tests (@institute.dev → @aldirasat.com)

**4. `apps/erp/backend/test_v1_7_full_e2e.py`** — ~25 occurrences
Replace all `@institute.dev` with `@aldirasat.com`
...
+ r = client.post('/api/v1/auth/login', json={'email': 'superadmin@aldirasat.com', ...})
```

**8. `apps/erp/backend/check_sections.py:4`**
```
- r = c.post('/api/v1/auth/login', json={'email': 'superadmin@institute.dev', ...})
+ r = c.post('/api/v1/auth/login', json={'email': 'superadmin@aldirasat.com', ...})
```

### Phase 3: Frontend E2E Tests (@institute.dev → @aldirasat.com)
...
Replace all `@institute.dev` → `@aldirasat.com`

### Phase 4: Documentation (lims.institute.local → aldirasat.edu)
